...
func (c *clientHttpCodec) ReadResponseHeader(r *rpc.Response) error {
	...
	if len(f[1]) != 3 {
		err = fmt.Errorf("malformed HTTP status code: %v, reason phrase: %v", f[1], reasonPhrase)
		return err
	}
	statusCode, err := strconv.Atoi(f[1])
	if err != nil || statusCode < 0 || statusCode != 200 {
		err = fmt.Errorf("malformed HTTP status code: %v", statusCode)
		return err
	}
	proto := f[0]
	if _, _, ok := http.ParseHTTPVersion(proto); !ok {
		err = fmt.Errorf("malformed HTTP version: %v", proto)
		return err
	}
...
func DialHTTPTimeout(urlStr string, timeout time.Duration, packagerName string) (*rpc.Client, error) {
	u, err := url.Parse(urlStr)
	if err != nil {
		return nil, err
	}
